...
=head1 SYNOPSIS
    ...
    sub show_inventory {
        ...
        foreach my $item ( @{$self->fruit_aisle} ) {
            print qq(Item: ) . blessed($item) . q(; name: ) 
                . $item->name . q(; species: ) . $item->species 
                . qq(\n);
...
reference:
    ...
    $store->show_inventory;
    # First inventory:
    # Item: Fruit; name: apple; species: M. domestica
    # Item: Fruit; name: orange; species: C. sinensis
    ...
    $store->show_inventory;
    # Second inventory:
    # Item: Fruit; name: grape; species: V. vinifera
    # Item: Fruit; name: tomato; species: S. lycopersicum
