...
The Perl 5 object has one variable, I<script_name>.  It's a good programming
practice to always validate your inputs, so we have the Perl 5 object check to
make sure that the user passes in the I<script_name> attribute to it when the
object is created.  The Moose object automatically checks this for us when we
set I<required =E<gt> 1> in the 'has' function for the Moose object.

=head3 FIXME - Verify this is how things behave
...
allowed to interact with the object using the interfaces that the object
provides.  One gotcha with Moose is that you must use the I<is> keyword to
have Moose create object accessors.  This is mentioned in
L<Moose::Cookbook::WTF>, in the section labeld B<Accessors>, but briefly:
...
=head3 Difference #5 - Referring to Moose object attributes
